# `elltest`: An R-package for implementing the $\ell$-test
...
methods in the Gaussian linear model, having exactly the same guarantees and showing significant power gains when the coefficient vector is not too dense. In fact, the alternative to the $t$-test, called the $\ell$-test, achieves power close to the one-sided $t$-test
